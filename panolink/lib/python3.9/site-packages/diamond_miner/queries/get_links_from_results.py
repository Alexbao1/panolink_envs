...
class GetLinksFromResults(ResultsQuery):
    ...
    def statement(
        ...
        return f"""
        WITH
            arrayStringConcat(arraySlice(splitByChar('.', toString(probe_dst_addr)), 1, 2), '.') || '.0.0' AS dst_prefix,
        ...
        SELECT
            ...
            if(link.3.3 = '-1' OR link.3.3 = ''
                , if(link.4.3 != '-1' AND link.4.3 != ''
                    , link.4.3
                    , dst_prefix
                  )
                , link.3.3
            ) AS near_asn, 
            if(link.4.3 != '-1', link.4.3, dst_prefix) AS far_asn
