...
from diamond_miner.queries.query import Query, StoragePolicy, rtt_table
from diamond_miner.typing import IPNetwork


@dataclass(frozen=True)
class CreateRttTable(Query):
    ...
    def statement(
        ...
        return f"""
        CREATE TABLE IF NOT EXISTS {rtt_table(measurement_id)}
        (
            probe_dst_addr         IPv6,
            probe_src_port         UInt16,
            probe_ttl              UInt8,
            recv_cnt               UInt8,
            median_rtt             UInt16 CODEC(T64, ZSTD(1)),
            reply_src_cnt          UInt8,
            round                  UInt16       
        )
        ENGINE MergeTree
        ORDER BY (probe_dst_addr, probe_src_port, round)
